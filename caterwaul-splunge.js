...
;return( b) .transform_v_with( translate(dv))} , context_box=function(b) { 
;return function(c) { 
;return( ( c.translate(b.v[0] , b.v[1]) , c.scale(b.dv[0] , b.dv[1])) , c)}} , zoom=function(b, dv) { 
;return( b) .transform_dv_with( scale(dv))} , centered_in=function(c) {var w= +c.width
;var h= +c.height
;var m=Math.min( w, h) 
...
;return this.slice(this.transform_, this.path_, this.view_.interpolate(c.view_, x) , this.slice_.interpolate(c.slice_, x) , this.data_.interpolate(c.data_, x))} , transformed_data:function() { 
...
;return(function(it) {return c.lineWidth /= this.view_.dv[0] ,it}) .call(this, ( context_box(this.view_) (c)))} , with_context:function(c, f) { 
...
;return find_point(this.view_.inverse() .transform(v) , this.transformed_data())} , delta:function(v) { 
;return transformed_delta(composite(this.view_, this.transform_, this.slice_) .inverse() , v)} , slice_delta:function(dv, f) { 
;return this.slice(f(this.slice_, dv))}}) ,it}) .call(this, ( (function(transform, path, view, slice, data) {return this.transform_ = transform, this.path_ = path, this.view_ = view, this.slice_ = slice, this.data_ = data, null}))) , rectangular_chart=function(data, options) {var options=$.merge( {} , rectangular_defaults, options) 
