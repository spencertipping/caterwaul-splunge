...
var m =Math.min( w, h) ;
 ...
 return Math.abs( box.dv[1] * (2 * box.v[0] * dr + dr * dr))} , rectangular_defaults = {transform:composite( bounding_box(box( [ -1, -1] , [2, 2 - epsilon])) , x_arctangent) , slice:scale( [1, 1]) , area:function (_) {return _.area()}} , radial_defaults = {transform:composite( polar_to_cartesian, box( [0.5, 0] , [0.5, tau - 0.001]) , bounding_box(box( [ -1, 0] , [2, 1])) , x_arctangent) , slice:scale( [1, 1]) , area:function (_) {return polar_area(_.transform_with(cartesian_to_polar))}} ;
