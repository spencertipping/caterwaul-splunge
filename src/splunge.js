...
return(function() {var sx=0,sy=0,start=null,last_selected=null,doc=jQuery(document) ,self=jQuery(canvas) ,setup_drag=function(x,y,v) {;
return( (sx=x,sy=y) ,start=v) } ,start_dragging=function(e) {;
return(setup_drag(e.pageX,e.pageY,self.val() ) , ( (doc) .mousemove(drag) ) .mouseup(stop_dragging) ) } ,stop_dragging=function(e) {;
...
return self.trigger( 'splunge_drag_delta' , {shift:e.shiftKey,ctrl:e.ctrlKey,alt:e.altKey,meta:e.metaKey,start:start,p1: [sx-x,sy-y] ,p2: [e.pageX-x,e.pageY-y] } ) } ) .call(this) } ;
...
var h=c.height() ;
var ar=w/h;
return viewport(data,options.zoom||translate( [0,0] ) ,polar_to_cartesian,options.logical_bound||box( [ -ar/2, -1/2] , [ar/w,1/h] ) ) } ,default_interaction=function(canvas,v) {;
