...
for (var xr = new xs.constructor() , xi = 0, xl = xs.length;
 ...
 return(function (it) {return it.untransformed_ =this, it}) .call(this, ( ( this) .map_corners(function (_) {return(t) .transform( _)})))} , scale:function (x) { ;
...
for (var xi = 0, xl = xs.length;
 ...
 return function (c) { ;
 return( ( ( c.beginPath() , c.arc(0, 0, p.v[0] , p.v[1] , p.v[1] + p.dv[1] , true)) , c.arc(0, 0, p.v[0] + p.dv[0] , p.v[1] + p.dv[1] , p.v[1])) , c.closePath())}} , transformed_delta =function (t, v) {var tv = t.transform(v) ;
 return function (dv) { ;
...
var m =Math.min( w, h) ;
 ...
 return Math.abs( box.dv[1] * (2 * box.v[0] * dr + dr * dr))} , rectangular_defaults = {transform:composite( bounding_box(box( [ -1, -1] , [2, 2 - epsilon])) , x_arctangent) , slice:scale( [1, 1]) , area:function (_) {return _.area()}} , radial_defaults = {transform:composite( polar_to_cartesian, box( [0.5, 0] , [0.5, Math.PI]) , bounding_box(box( [ -1, -1 + 1e-4] , [2, 2 - 2e-4])) , x_arctangent) , slice:scale( [1, 1]) , area:function (_) {return polar_area(_.transform_with(cartesian_to_polar))}} ;
