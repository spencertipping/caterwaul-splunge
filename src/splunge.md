...
TODO: Rewrite all of this. I should have thought of it sooner, but using recursive reduce() calls limits the number of data elements that can be processed at all.
...
though they will work just fine (albeit inefficiently) for small finite datasets. You should use bounded(), x_shadow(), or y_shadow() for large or infinite data sets.
    ...
    find_point(v, xs)       = descend_while("_.bound().contains(v)".qf, xs) /!first,
