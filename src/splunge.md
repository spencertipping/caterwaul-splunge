...
List data is designed to be transformed incrementally. Traversal is expressed as a lazy reduction. For example:

    length(s)   = s.reduce(0, given[x, rest][1 + rest()])       // this forces the whole list
    identity(s) = s.reduce(null, cons)                          // null marks the end of a list
