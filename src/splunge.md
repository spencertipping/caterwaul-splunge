...
<canvas> element and allowing the user to change it. Note that centered_in() takes just a regular <canvas> element, not a jQuery object.
      ...
      radial_defaults                 = {transform: polar_to_cartesian / box([0.5, 0], [0.5, Math.PI]) / bounding_box([-1, -1 + 1e-4] /-box/ [2, 2 - 2e-4]) /-composite/ x_arctangent,
                                         slice:     [1, 1] /!scale,
                                         area:      "polar_area(_.transform_with(cartesian_to_polar))".qf}],
