...
children all of whose bounds are contained within the bounding box.
    ...
    bounded(xs)                           = capture [bound() = bound, children() = children, transform_with(t) = bounded(this.children() *[x.transform_with(t)] -seq),
                                                                                             interpolate(b, x) = bounded_interpolation(this, b, x)]
                                             -where [bound    = xs *[x.bound()] /[x0 /~union/ x] -seq,
    ...
    descend_while(f, x)                   = f(x) ? [x] + x.children() *~![descend_while(f, x)] -seq : [],
