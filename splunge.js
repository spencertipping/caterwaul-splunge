...
for (var xr = new xs.constructor() , xi = 0, xl = xs.length;
 ...
 return xr}) .call(this,Array.prototype.slice.call( (ts))))))}}} , box_ctor = (function (it) {return $.merge( it.prototype, {intersect:function (b) { ;
 return( this) .map_corners(function (_) {return(b) .intern( _)})} , contains:function (v) { ;
 return v[0] >= this.v[0] && v[1] >= this.v[1] && v[0] <= this.v[0] + this.dv[0] && v[1] <= this.v[1] + this.dv[1]} , interpolate:function (b, x) { ;
 return( this.scale(1 - x)) .plus( b.scale(x))} , area:function () { ;
 return Math.max(0, this.dv[0]) * Math.max(0, this.dv[1])} , transform:function (v) { ;
 ...
 return rectangle(this.data, ( t) .transform( this.v) , this.dv)} , transform_dv_with:function (t) { ;
 return rectangle(this.data, this.v, ( t) .transform( this.dv))} , children:function () { ;
...
for (var rr = new rs.constructor() , ri = 0, rl = rs.length;
 ...
 return bounded_interpolation(this, b, x)} , transform_with:function (t) {var self = this;
 return cached(lazy_bounded(function (_) {return(function (xs) {var x, x0, xi, xl, xr;
for (var xr = new xs.constructor() , xi = 0, xl = xs.length;
 xi < xl;
 ++xi) x = xs[xi] , xr.push( (x.transform_with(t))) ;
 return xr}) .call(this,self.children())} , t))}})} , bound_union =function (xs) { ;
 return(function (xs) {var x, x0, xi, xl, xr;
for (var x0 = (null) , xi = 0, xl = xs.length;
 xi < xl;
 ++xi) x = xs[xi] , x0 = (x0 ? ( x0) .union( x.bound()): x.bound()) ;
 return x0}) .call(this, xs) || bound_nothing} , bounded =function (xs) { ;
 return cached(lazy_bounded(function (_) {return xs}))} , cached =function (b) { ;
 return renderable( {bound:function () { ;
 return b.bound()} , children:function () { ;
 return this.children_ !== void 0 ? this.children_: this.children_ = b.children()}})} , lazy_bounded =function (f, t) { ;
 return renderable( {bound:function () { ;
 return this.bound_ || (this.children_ ? this.bound_ !== void 0 ? this.bound_: this.bound_ =bound_union( this.children_): t ? ( bound_everything) .transform_with( t): bound_everything)} , children:function () { ;
 return f()}})} , scale_size =function (l, x) {var b = l.bound() ;
...
var m =Math.min( w, h) ;
 ...
 return new chart_ctor(options.transform, arc_path, options.area, options.view, options.slice, data)} , polar_area =function (box) {var dr = Math.max(0, box.dv[0]) ;
 return Math.abs( Math.max(0, box.dv[1]) * (2 * box.v[0] * dr + dr * dr))} , rectangular_defaults = {transform:composite( bounding_box(box( [ -1, -1] , [2, 2 - epsilon])) , x_arctangent) , slice:scale( [1, 1]) , area:function (_) {return _.area()}} , radial_defaults = {transform:composite( polar_to_cartesian, scale( [1, Math.PI]) , bounding_box(box( [0, -1] , [1, 2 - epsilon])) , x_arctangent) , slice:scale( [1, 1]) , area:function (_) {return polar_area(_.transform_with(cartesian_to_polar))}} ;
 return{ tau: tau, atan_scale: atan_scale, scaled_atan: scaled_atan, clip: clip, epsilon: epsilon, infinity: infinity, scaled_tan: scaled_tan, componentwise: componentwise, x_tangent: x_tangent, x_arctangent: x_arctangent, y_tangent: y_tangent, y_arctangent: y_arctangent, polar_to_cartesian: polar_to_cartesian, cartesian_to_polar: cartesian_to_polar, bounding_box: bounding_box, identity_transform: identity_transform, composite: composite, box_ctor: box_ctor, box: box, translate: translate, bound_everything: bound_everything, rectangle: rectangle, scale: scale, bound_nothing: bound_nothing, x_stack: x_stack, y_stack: y_stack, renderable: renderable, bound_union: bound_union, bounded: bounded, cached: cached, lazy_bounded: lazy_bounded, scale_size: scale_size, bounded_interpolation: bounded_interpolation, find_point: find_point, descend_while: descend_while, no_path: no_path, rectangle_path: rectangle_path, arc_path: arc_path, transformed_delta: transformed_delta, pan: pan, context_box: context_box, zoom: zoom, centered_in: centered_in, chart_ctor: chart_ctor, rectangular_chart: rectangular_chart, radial_chart: radial_chart, polar_area: polar_area, rectangular_defaults: rectangular_defaults, radial_defaults: radial_defaults}}) .call(this)}) .call(this)}) ;
