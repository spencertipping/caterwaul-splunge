...
return{x:Math.tan( (Math.sqrt(p.x*p.x+p.y*p.y) -0.5) *Math.PI*t.dx+t.x0) ,y:Math.atan2(p.x,p.y) *t.dy+t.y0} } ,transform=function(t,p) {;
...
return(function(it) {return it&&p.x<=it.x0+it.dx} ) .call(this, (point_hits_desc(p,h) ) ) } ,arc_path=function(h,t) {;
return function(c) {;
